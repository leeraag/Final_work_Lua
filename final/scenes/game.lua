...
--local rect = display.newRect(10, 10, 30, 1050)
...
local function spawnBomb ()
  ...
  bomb.width = w-90
  bomb.myName = "bomb"
  physics.addBody(bomb, "dynamic", {radius = 10, isSensor = true})
  end
  spawnBomb_timer = timer.performWithDelay (500,spawnBomb, 0)
  ...
  spawnPlus_timer = timer.performWithDelay (1000,spawnPlus, 0)
  ...
  spawnPlus5_timer = timer.performWithDelay (5000,spawnPlus5, 0)
  ...
  spawnHeal_timer = timer.performWithDelay (7000,spawnHeal, 0)

--игрок
local player = display.newImage(sceneGroup, tst, 100, 470)
player.height = h
player.width = w
...
local function onPlayAgainTouch()
        composer.gotoScene("scenes.menu", "fade") -- возврат в меню
    end
...
function scene:show( event )
  ...
  --if (phase == "will") then --перед первой загрузкой
...
function scene:hide( event )
  --[[local phase = event.phase
    if (phase == "will") then --перед закрытием
        physics.stop() 
        physics = nil
    end
    if (phase == "did") then --когда сцена уже закрыта
